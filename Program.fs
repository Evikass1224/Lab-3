...
let FileNumbers (filePath: string) : float seq =
    seq {
        use reader = new StreamReader(filePath)   //чтение файла
        while not reader.EndOfStream do           //пока не конец файла
            let line = reader.ReadLine()          //чтение строки
